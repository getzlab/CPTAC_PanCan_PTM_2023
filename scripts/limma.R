...
RunLimmaTest <- function(mat.df, var.df, covariates.df=NULL, genes.df=NULL, use.sva=TRUE, n.sv=NULL, return_mod=FALSE, minObs=10) {
    ...
    # Subset matrix for minimum threshold of sample observations
    bfeats <- dim(mat.df)[[1]]
    mat.df <- mat.df[which( (rowSums(is.na(inGroup.mat)) <= dim(inGroup.mat)[[2]]-minObs) & (rowSums(is.na(outGroup.mat)) <= dim(outGroup.mat)[[2]]-minObs) ), ]
    cat(paste("\n    * ",dim(mat.df)[[1]], "/", bfeats, "kept for NA filt\n"))
    cat(paste("minObs: ", minObs, "\n"))
    if (use.sva) {
    ...
    cat(paste0("Differentially expressed genes at 0.05 FDR: ", sum(res[which(!is.na(res$qval)), 'qval']<=0.05), "\n"))
    if (!is.null(genes.df)) {
...
RunDiffExp <- function(mat.df, labels.df, id, map.df, covar.df=NULL, continuous=TRUE, minObs=10){
    ...
    cat(paste("minObs RunDiffExp(): ", minObs, "\n"))
